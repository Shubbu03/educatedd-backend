...
export class MediaController {
  ...
  public async createMedia(
    ...
    const createdMedia: MediaUseCaseDto = await this.createMediaUseCase.execute(
      adapter
    );
  ...
  public async editMedia(
    ...
    const editedMedia: MediaUseCaseDto = await this.editMediaUseCase.execute(
      adapter
    );
  ...
  public async getMediaList(
    ...
    const medias: MediaUseCaseDto[] = await this.getMediaListUseCase.execute(
      adapter
    );
  ...
  private setFileStorageBasePath(medias: MediaUseCaseDto[]): void {
    medias.forEach(
      (media: MediaUseCaseDto) =>
        (media.url = resolve(FileStorageConfig.BASE_PATH, media.url))
    );
  }
}
